...
def realize(params):
    ...
    PH = PHert(dip=params[1], H=params[2], xpos=params[3], rho_fault=params[5], rho_back=params[5], dep=params[6], xtra=params[7], Q=params[8], outfile=real)
    PH.run_full()
    return
...
class MonteCarlo:
    ...
    def prepare_realization(self,i):
        ...
        PH = PHert(dip=self.PARAMS[i][1], H=self.PARAMS[i][2], xpos=self.PARAMS[i][3], rho_fault=self.PARAMS[i][5], rho_back=self.PARAMS[i][5], dep=self.PARAMS[i][6], xtra=self.PARAMS[i][7], Q=self.PARAMS[i][8], outfile=real)
        return PH
